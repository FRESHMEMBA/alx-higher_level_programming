...
def matrix_divided(matrix, div):
    ...
    Parameters:
    - matrix (list of lists): The matrix to be divided. Each element of the
    matrix should be an integer or a float.
    - div (int or float): The divisor to divide the matrix elements by.

    Returns:
    - matrix_div (list of lists): The resulting matrix after dividing each
    element by the divisor. Each element of the resulting matrix will be
    rounded to 2 decimal places.

    Raises:
    - TypeError: If the matrix is not a list of lists, or if any element
    of the matrix is not an integer or a float.
